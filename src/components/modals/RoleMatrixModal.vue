<template>
  <div>
    <div class="flex items-center justify-between p-4 border-b border-outline-variant">
      <h3 class="text-headline-sm font-headline-md text-on-surface">Role Matrix</h3>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider">
            <th class="p-2 border-b border-outline-variant">Role</th>
            <th class="p-2 border-b border-outline-variant">Tickets</th>
            <th class="p-2 border-b border-outline-variant">Inventory</th>
            <th class="p-2 border-b border-outline-variant">Users</th>
            <th class="p-2 border-b border-outline-variant">Reports</th>
            <th class="p-2 border-b border-outline-variant">Settings</th>
          </tr>
        </thead>
        <tbody class="text-body-sm text-on-surface">
          <tr v-for="row in matrix" :key="row.role" class="border-b border-outline-variant/30">
    ...
    <div class="flex justify-end p-4 border-t border-outline-variant">
      <button @click="$emit('close')" class="px-4 py-2 border border-outline-variant rounded text-label-md text-on-surface hover:bg-surface-container">Close</button>
    </div>
  </div>
</template>
...
function badgeClass(val) {
  if (val === 'Full') return 'px-2 py-0.5 rounded text-label-sm bg-green-100 text-green-800'
  if (val === 'Edit' || val === 'Create') return 'px-2 py-0.5 rounded text-label-sm bg-blue-100 text-blue-800'
  if (val === 'View') return 'px-2 py-0.5 rounded text-label-sm bg-surface-container text-on-surface-variant'
  return 'px-2 py-0.5 rounded text-label-sm bg-surface-container text-on-surface-variant'
}
</script>
