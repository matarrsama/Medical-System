  ...
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant dark:border-outline shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="text-headline-md font-headline-md text-on-surface dark:text-inverse-on-surface">Create New Request</h3>
          <p class="text-body-sm text-on-surface-variant dark:text-outline mt-0.5">Submit an equipment, access, or service request</p>
        </div>
      </div>
    ...
    <form @submit.prevent="submit" class="flex flex-col flex-1 min-h-0">
      <div class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
        <div class="flex items-center gap-3">
          <label class="text-label-md font-label-md text-on-surface-variant dark:text-outline">Request ID</label>
        ...
        <div>
          ...
          <input v-model="form.title" placeholder="e.g. ICU Monitor Calibration" class="w-full px-3 py-2.5 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary focus:border-primary transition-colors" required />
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="text-label-md font-label-md text-on-surface dark:text-inverse-on-surface mb-1.5 block">Type <span class="text-error">*</span></label>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label class="text-label-md font-label-md text-on-surface dark:text-inverse-on-surface mb-1.5 block">Location <span class="text-on-surface-variant dark:text-outline text-label-sm">(optional)</span></label>
            <input v-model="form.location" placeholder="e.g. Ward 3B, Room 12" class="w-full px-3 py-2.5 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary focus:border-primary transition-colors" />
          </div>
        </div>

        <div>
          <label class="text-label-md font-label-md text-on-surface dark:text-inverse-on-surface mb-1.5 block">Description</label>
          <textarea v-model="form.description" rows="4" placeholder="Describe what is needed and any relevant details..." class="w-full px-3 py-2.5 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary focus:border-primary transition-colors resize-none"></textarea>
        </div>
...
const departmentLocked = computed(() =>
  !!authStore.departmentHeadOf && !authStore.canManageServiceRequests
)
...
async function submit() {
  ...
  try {
    ...
    toast.error(mapFirebaseError(err, 'Failed to submit request.'))
  } finally {
    saving.value = false
  }
}
