  ...
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant dark:border-outline shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="text-headline-md font-headline-md text-on-surface dark:text-inverse-on-surface">{{ editing ? 'Edit Equipment' : 'Equipment Details' }}</h3>
          <p class="text-body-sm text-on-surface-variant dark:text-outline mt-0.5">{{ equipment.equipmentId || equipment.id }}</p>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto px-6 py-5 space-y-6">
      <div class="flex items-start gap-4">
        <div class="min-w-0 flex-1">
          ...
          <template v-else>
            <h2 class="text-headline-sm font-headline-md text-on-surface dark:text-inverse-on-surface">{{ equipment.name }}</h2>
          </template>
      ...
      <div class="grid grid-cols-2 gap-x-6 gap-y-4 p-4 rounded-xl bg-surface-container-low dark:bg-inverse-surface">
        <div>
          ...
          <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5 font-mono">{{ equipment.equipmentId || equipment.id }}</p>
        </div>
        <div>
          ...
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.type || '—' }}</p>
          </template>
        </div>
        <div>
          ...
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.department || '—' }}</p>
          </template>
        </div>
        <div>
          ...
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.status || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Manufacturer</span>
          <template v-if="editing">
            <input v-model="editForm.manufacturer" class="w-full mt-0.5 px-3 py-2 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary" />
          </template>
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.manufacturer || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Model</span>
          <template v-if="editing">
            <input v-model="editForm.model" class="w-full mt-0.5 px-3 py-2 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary" />
          </template>
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.model || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Serial Number</span>
          <template v-if="editing">
            <input v-model="editForm.serialNumber" class="w-full mt-0.5 px-3 py-2 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary" />
          </template>
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.serialNumber || '—' }}</p>
          </template>
        </div>
        <div>
          ...
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.location || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Last Calibration</span>
          <template v-if="editing">
            <input v-model="editForm.lastCalibration" type="date" class="w-full mt-0.5 px-3 py-2 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary" />
          </template>
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.lastCalibration || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Next Calibration</span>
          <template v-if="editing">
            <input v-model="editForm.nextCalibration" type="date" class="w-full mt-0.5 px-3 py-2 border border-outline-variant dark:border-outline rounded-lg text-body-sm bg-surface dark:bg-inverse-surface focus:ring-1 focus:ring-primary" />
          </template>
          <template v-else>
            <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.nextCalibration || '—' }}</p>
          </template>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Created by</span>
          <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ equipment.createdByName || equipment.createdBy || '—' }}</p>
        </div>
        <div>
          <span class="text-label-sm text-outline font-medium">Created</span>
          <p class="text-body-md text-on-surface dark:text-inverse-on-surface font-medium mt-0.5">{{ formatDate(equipment.created) }}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <template v-else>
          <p class="text-body-md text-on-surface dark:text-inverse-on-surface bg-surface-container-low dark:bg-inverse-surface rounded-xl p-4 whitespace-pre-wrap">{{ equipment.description || 'No description provided.' }}</p>
        </template>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-outline-variant dark:border-outline shrink-0 bg-surface-container-low dark:bg-inverse-surface">
      ...
      <template v-else>
        <button v-if="canManage" @click="deleteEquipment" class="flex items-center gap-2 px-5 py-2.5 rounded-lg text-label-md font-label-md text-error hover:bg-error-container/20 transition-colors">
        ...
        <button v-if="canManage" @click="startEdit" class="flex items-center gap-2 px-6 py-2.5 rounded-lg text-label-md font-label-md text-on-primary bg-primary hover:bg-primary-container transition-all shadow-sm">
...
const canManage = computed(() => (authStore.canEditEquipment || authStore.canDeleteEquipment) || !!authStore.departmentHeadOf)
...
function typeClass(t) {
  const map = { Diagnostic: 'bg-surface-container-highest text-on-surface-variant dark:text-outline', Therapeutic: 'bg-tertiary-container/20 text-tertiary', Monitoring: 'bg-primary-container/30 text-primary dark:bg-primary-container/40 dark:text-inverse-primary', Laboratory: 'bg-surface-container text-on-surface-variant dark:text-outline', Surgical: 'bg-error-container/30 text-on-error-container dark:bg-error-container/40', Imaging: 'bg-secondary-container/30 text-secondary', Other: 'bg-surface-container-highest text-on-surface-variant dark:text-outline' }
  return map[t] || ''
}

function statusClass(s) {
  const map = { Operational: 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200', 'Needs Calibration': 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200', 'Out of Service': 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' }
  return map[s] || ''
}
