<template>
  <div class="flex flex-col">
    <div class="flex items-center justify-between px-5 py-4 border-b border-outline-variant">
      <h3 class="text-headline-sm font-headline-md text-on-surface">Reset Password</h3>
    ...
    <div class="px-5 py-6 space-y-5">
      <div class="flex items-center gap-3.5">
        ...
        <div class="min-w-0">
          <p class="text-body-md font-medium text-on-surface truncate">{{ user.name }}</p>
          <p class="text-body-sm text-on-surface-variant truncate">{{ user.email }}</p>
        </div>
      </div>
      <div class="p-4 rounded-xl bg-surface-container-low space-y-3">
        ...
        <div class="flex items-center gap-2 bg-surface-container-highest rounded-lg px-3.5 py-3">
          ...
          <button @click="copyPassword" class="w-8 h-8 rounded-lg bg-surface-container-low text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all shrink-0">
        ...
        <div class="flex items-center gap-2 text-body-sm text-on-surface-variant">
          ...
          <span>Password sent to <strong class="text-on-surface">{{ user.email }}</strong></span>
        </div>
      </div>
    </div>
    <div class="flex justify-end px-5 py-4 border-t border-outline-variant">
